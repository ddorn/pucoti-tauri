...
- **Hold a timer when something interrupts you**: press <kbd>Shift</kbd>+<kbd>Enter</kbd> to start a new timer without losing the one you were on. The old timer goes on hold — its countdown freezes and its bell goes quiet — and comes back the moment you finish the new one with <kbd>Enter</kbd> or bin it with <kbd>q</kbd>. Time spent on hold doesn't count against its prediction.
  - There is no way to switch between held timers on purpose: the only way back to what you were doing is to finish or bin the thing on top of it.
  - You can have three timers going at once by default. Change it under Settings → Timer → "Timers at once", or set it to 1 for one timer at a time, as before.
  ...
  - After you finish a timer, the held one stays frozen while you look at your completion stats, and picks up again when you return to the timer.

### Changed

- **Prefill now runs whenever you start a timer**, instead of needing <kbd>Shift</kbd>+<kbd>Enter</kbd>. The text arrives selected, so typing replaces it, and <kbd>Tab</kbd> fetches it again if you cleared it. It no longer blocks the input while the command runs — but note it now runs on every timer you start, not only when you ask for it.
...
- **Timers with a duration and no intent can be completed**: starting one with <kbd>Shift</kbd>+<kbd>Enter</kbd> (like `25m` with no text) left <kbd>Enter</kbd> opening the intent input instead of finishing the timer.
